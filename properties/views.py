...
def azizi_florence(request):
    """
    Azizi Florence, Dubai — community + project SEO landing page. Content is
    editorial/static (built for organic ranking on "Azizi Florence Dubai" and
    ...
    record via AZIZI_FLORENCE_PROPERTY_SLUG.
    """
    property_obj = (
        Property.objects
        .filter(slug=AZIZI_FLORENCE_PROPERTY_SLUG, is_active=True)
        .first()
    )
    ...
    if request.method == 'POST':
        ...
        if property_obj is None:
            # Fails closed rather than 500ing or silently dropping the lead.
            logger.error(
                'Azizi Florence landing page: AZIZI_FLORENCE_PROPERTY_SLUG "%s" not found — '
                'set it to a real Property slug before enquiries can be saved.',
                AZIZI_FLORENCE_PROPERTY_SLUG,
            )
            messages.error(
                request,
                'Sorry, enquiries are temporarily unavailable. Please call or '
            ...
            enquiry = enquiry_form.save(commit=False)
            enquiry.property = property_obj
            enquiry.source = 'Azizi Florence Landing Page'
            enquiry.save()
            ...
            for sender, label in (
                ...
                try:
                    ...
                    logger.error('Azizi Florence enquiry %s email failed: %s', label, exc)
            ...
            return redirect(f"{reverse('properties:azizi_florence')}#enquiry")
        else:
            messages.error(request, 'Please correct the highlighted fields and try again.')
    ...
    # display windows.
    meta_title, meta_description, seo_report = _audit_seo(
        property_obj if property_obj else Property(pk=0, title='Azizi Florence Dubai'),
        'Azizi Florence Dubai | Apartments By Azizi Developments',
        'Discover Azizi Florence Dubai — modern apartments with lifestyle amenities '
        'and strong investment potential by Azizi Developments.',
    )

    return render(request, 'aziziflorence.html', {
        'property': property_obj,
        'enquiry_form': enquiry_form,
